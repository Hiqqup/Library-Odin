...
const domElements = {
    ...
    displayBook: function(book){
        ...
        bookHtml+= book.author? `<p class="author">by <b>${book.author}</b></p>`: '<p class="author">No author specified</p>';
        bookHtml+=  `<p class="pages">with <b>${book.pages}</b> pages</p>`;
        bookHtml += `<p>Read: <input class="readBox" type="checkbox" onchange="updateBook('${book.id}')"`+ (book.read?" checked " : "")+ "></p>"
        ...
        if(book.uploadedCover){
            const imageContainer = document.getElementById( book.id).querySelector(".imageContainer");
...
function updateBook(bookId){
    ...
    library[bookId].read = bookContainer.querySelector(".readBox").checked;
    library[bookId].currentPage = parseInt(bookContainer.querySelector(".currentPage").value);
    bookContainer.querySelector(".readingProgress").value = library[bookId].currentPage/ library[bookId].pages;
    storeChange();
}
